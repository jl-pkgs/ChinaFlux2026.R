...
Met <- fread("/mnt/z/GitHub/jl-pkgs/ChinaFlux2026/data/BEPS/Forcing_Hourly_Met_sp31_v20260629.csv") # hourly
# Met[Rs > 1370, .N, site] # 风速不应为负
# Met[site == "SAV_荒漠草原_达茂" & WS_canopy > 100, ] 

# 瞬时越界值置 NA（见 R/check_bounds.R）
Met <- rename_forward(Met) %>% check_bounds(scale = "hourly") %>% rename_backward()
...
tab <- merge(met_agg, flux_agg, by = "site", all.x = TRUE) %>%
  ...
  arrange(site)
fwrite(tab, file.path(outdir, "missing_input_China_FluxALL.csv"), bom = TRUE)
...
# )]
fwrite(ann, file.path(outdir, "annual_unit_check.csv"))
