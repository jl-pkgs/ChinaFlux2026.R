...
VARS_BOUNDS_DAILY <- list(
  ...
  GPP       = c(  0,  30),     # gC m-2 d-1 日均总初级生产力
...
VARS_BOUNDS_HOURLY <- list(
  ...
  Rs        = c(  0, 1370),   #! W/m2 入射短波（瞬时晴空峰值）, 1361 W/m²
  ...
  # 系数 = 1e-6 (µmol→mol) × 12.011 (gC/mol) × 86400 (s/d) ≈ 1.0378
  NEE       = c( -70,  50),   # µmol CO2 m-2 s-1, 瞬时净生态系统交换通量（单位随数据而定）
  GPP       = c(   0,  60),   # µmol CO2 m-2 s-1, 日均总初级生产力
